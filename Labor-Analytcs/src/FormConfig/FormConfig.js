export default class FormHandler {
  ...
  setFormValue = (name, value) => {
    ...
    if (type === "file" && value) {
      ...
      reader.onloadend = () => {
        this.formInputs[name].value = reader.result;
        this.formInputs[name].valueFile = value;
  ...
  getFieldValue = (name) => {
    if (this.formInputs[name]) {
      const { type, getFileValue } = this.formInputs[name];
      return type === "file" && getFileValue
  ...
  testErrorsAndReturnData = () => {
    ...
    this.inputNames.map((name) => {
      const { maskToSubmit } = this.formInputs[name];
      const value = String(this.getFieldValue(name)).trim();
      if (Boolean(value) || value === 0) {
        data[name] = this.formatValueToSubmit(value, maskToSubmit);
      }
