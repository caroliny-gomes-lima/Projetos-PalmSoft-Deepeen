...
function BlockedModal({ userData, isFetching, EnabledUsersBlock }) {
  const text = Texts["pt-BR"].users.enabledUsers.blockedModal;
...
function mapDispatchToProps(dispatch) {
  const { EnabledUsersBlock } = Creators;
  return {
    EnabledUsersBlock: function (userId) {
      return dispatch(EnabledUsersBlock(userId));
    },
  };
}
export default connect(mapStateToProps, mapDispatchToProps)(BlockedModal);
